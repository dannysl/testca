...
from authy.CaptchaResponse import CaptchaResponse
...
class CaptchaClient:
    ...
    def checkToken(self, token):
        ...
        sign = hashlib.md5((self.appSecret + arr[0] + self.appSecret).encode()).hexdigest()
        req_url = self.requestUrl + '?appKey=' + self.appId + '&token=' + arr[0] \
                  + '&constId=' + constId + "&sign=" + sign

        try:
            self.response =  urllib.request.urlopen(req_url, timeout = self.timeout)
